...
def main():
    # 加载缺失股票代码
    with open('/tmp/missing_codes.json', 'r') as f:
        codes = json.load(f)

    print(f"加载了 {len(codes)} 只缺失股票")
    ...
    # 使用新的进度文件，避免被旧的干扰
    fetcher.progress_file = '/Users/bruce/workspace/trade/StockData/status/backfill_missing.json'
